...
import { queryShards, queryConversationChunks, downloadShard, fetchIdentity, } from "./arweave.js";
...
export function buildShardTags(walletAddress, version, type, signature) {
    return [
        { name: "App-Name", value: "sharedcontext" },
 ...
 */
export async function pushShard(encryptedBlob, version, type, walletAddress, privateKey, backend) {
 ...
 */
export async function pullAndReconstruct(walletAddress, passphrase, dbPath) {
    ...
    if (!identity) {
        throw new Error("No identity found on Arweave for this wallet. " +
            "Make sure you ran `sharedcontext init` on another device and pushed the identity.");
    }
    ...
    const dataShards = allShards.filter((s) => s.type === "delta" || s.type === "snapshot");
    ...
    for (const shardInfo of shardsToFetch) {
        try {
            const encrypted = await downloadShard(shardInfo.txId, MAX_PULL_DATA_SHARD_BYTES);
            // Signature is mandatory for all data shards.
            if (!shardInfo.signature) {
                console.warn(`Skipping shard v${shardInfo.version}: missing signature.`);
                continue;
            }
            if (!verifySignature(encrypted, shardInfo.signature, walletAddress)) {
                console.warn(`Skipping shard v${shardInfo.version}: signature verification failed.`);
                continue;
            }
            ...
            console.warn(`Skipping shard v${shardInfo.version}: ${err instanceof Error ? err.message : String(err)}`);
            continue;
        }
    }
    if (decryptedShards.length === 0) {
        throw new Error("No valid shards could be recovered from Arweave (all candidates were invalid or unreadable).");
    }
 ...
 */
export async function pushConversationDelta(conversation, encryptionKey, walletAddress, privateKey, backend, lastSyncedCount) {
 ...
 */
export async function pullConversations(walletAddress, encryptionKey) {
    ...
    for (const group of grouped.values()) {
        try {
            ...
            const client = parsed.client === "cursor" || parsed.client === "claude-code"
                ? parsed.client
                : null;
    ...
    const perSessionOffsets = new Map();
    ...
    for (const seg of segmentPayloads) {
        const existingOffsets = perSessionOffsets.get(seg.session) ?? new Set();
        // If we already have this exact offset, keep the first one (stable by timestamp sort).
        if (existingOffsets.has(seg.offset))
            continue;
        ...
        if (seg.startedAt < existing.startedAt)
            existing.startedAt = seg.startedAt;
        if (seg.updatedAt > existing.updatedAt)
            existing.updatedAt = seg.updatedAt;
    }
    return Array.from(sessions.values()).sort((a, b) => a.updatedAt.localeCompare(b.updatedAt));
}
function groupConversationChunkInfos(infos) {
...
function findLatestSnapshot(shards) {
 ...
 */
export async function checkRemoteVersion(walletAddress) {
    const shards = await queryShards(walletAddress);
    const dataShards = shards.filter((s) => s.type === "delta" || s.type === "snapshot");
    if (dataShards.length === 0)
        return null;
