...
export function keychainStore(passphrase) {
    ...
    if (os === "darwin") {
        ...
        try {
            execFileSync("security", ["delete-generic-password", "-s", SERVICE, "-a", ACCOUNT, kc], { stdio: "ignore" });
        }
        catch {
    ...
    `.trim();
        execFileSync("powershell", ["-NoProfile", "-Command", ps], { stdio: ["pipe", "ignore", "ignore"], input: passphrase });
    }
    else {
...
export function keychainLoad() {
    ...
    try {
        if (os === "darwin") {
            const kc = loginKeychain();
            const result = execFileSync("security", ["find-generic-password", "-s", SERVICE, "-a", ACCOUNT, "-w", kc], { encoding: "utf-8", stdio: ["pipe", "pipe", "ignore"] });
            return result.trim();
        }
        else if (os === "linux") {
            const result = execFileSync("secret-tool", ["lookup", "service", SERVICE, "account", ACCOUNT], { encoding: "utf-8", stdio: ["pipe", "pipe", "ignore"] });
            return result.trim();
        }
        else if (os === "win32") {
            const ps = `
...
export function keychainDelete() {
    ...
    try {
        if (os === "darwin") {
            ...
            execFileSync("powershell", ["-NoProfile", "-Command", `Remove-StoredCredential -Target '${SERVICE}'`], { stdio: "ignore" });
        }
    }
    catch {
