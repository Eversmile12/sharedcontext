 ...
 */
export function createShard(operations, version, sessionId) {
    return {
 ...
 */
export function createChunkedShards(operations, startVersion, sessionId, maxEncryptedBytes = MAX_SHARD_BYTES) {
    if (operations.length === 0)
        return [];
    const maxJsonBytes = maxEncryptedBytes - ENCRYPTION_OVERHEAD;
    const chunks = chunkOperationsBySize(operations, maxJsonBytes);
    return chunks.map((ops, i) => createShard(ops, startVersion + i, sessionId));
}
/**
 ...
 */
function chunkOperationsBySize(operations, maxJsonBytes) {
    ...
    const wrapperSize = encoder.encode(JSON.stringify({
 ...
 */
export function deserializeShard(data) {
 ...
 */
export function replayShards(shards) {
    const state = new Map();
 ...
 */
export function factToUpsertOp(fact) {
    return {
        op: "upsert",
        fact_id: fact.id,
        key: fact.key,
