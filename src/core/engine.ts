 ...
 */
export const MODEL_BUDGETS: Record<string, { window: number; allocation: number }> = {
 ...
 */
function scoreFact(fact: Fact, keywords: string[]): number {
  // Tag match score: 10 points per matching tag
  const tagMatches = fact.tags.filter((tag) =>
  ...
  const daysSinceConfirmed =
    (Date.now() - new Date(fact.last_confirmed).getTime()) / (1000 * 60 * 60 * 24);
...
export function recallContext(
  ...
  // Tier 1: Scope filter
  const scopeFiltered = allFacts.filter(
    (f) => f.scope === "global" || f.scope === currentScope
  );
 ...
 * "project:sharme:storage:backend" → "storage backend"
 * "global:coding_style"           → "coding style"
 */
function simplifyKey(key: string, scope: string): string {
  let simplified = key;

  // Remove scope prefix: "project:sharme:" or "global:"
 ...
 */
export function formatContext(facts: Fact[]): string {
  if (facts.length === 0) return "";

  const lines = facts.map(
    (f) => `- ${simplifyKey(f.key, f.scope)}: ${f.value}`
  );
