...
import { deriveKeypairFromPhrase, publicKeyFromPrivate, addressFromPublicKey } from "../core/identity.js";
...
import type { Fact, ShardOperation } from "../types.js";
...
// Track shards created in the current session for display
...
let lastShards: ShardRecord[] = [];

// Track pushed TX IDs for reconstruction (simulates knowing only TX IDs + passphrase)
interface PushedTx {
  txId: string;
  type: "identity" | "delta";
  version: number;
}
let pushedTransactions: PushedTx[] = [];
...
let pendingConfirmIndices: [number, number] | null = null;

function handleGeneratePhrase(_req: IncomingMessage, res: ServerResponse): void {
...
async function handleInit(req: IncomingMessage, res: ServerResponse): Promise<void> {
  ...
  const { word1, word2 } = body as { word1?: string; word2?: string };
...
function handleGetFacts(_req: IncomingMessage, res: ServerResponse): void {
  if (!db) {
...
async function handleStore(req: IncomingMessage, res: ServerResponse): Promise<void> {
  ...
  const body = JSON.parse(await readBody(req));
  const { key, value, scope, tags } = body as {
    key: string;
    value: string;
    scope?: string;
    tags?: string[];
...
async function handleShard(_req: IncomingMessage, res: ServerResponse): Promise<void> {
  if (!db || !encryptionKey) {
    json(res, { error: "Not initialized" }, 400);
    return;
  }
...
function handleWipe(_req: IncomingMessage, res: ServerResponse): void {
  if (!db) {
    json(res, { error: "Not initialized" }, 400);
    return;
  }
...
async function handleReconstruct(req: IncomingMessage, res: ServerResponse): Promise<void> {
  ...
  const steps: Array<{ step: string; detail: string; data?: unknown }> = [];
  ...
  const tagJson = (await tagRes.json()) as {
    data: { transactions: { edges: Array<{ node: { tags: Array<{ name: string; value: string }> } }> } };
  };

  let saltHex: string | null = null;
  const edge = tagJson.data?.transactions?.edges?.[0];
  if (edge) {
    const tagMap = new Map(edge.node.tags.map((t: { name: string; value: string }) => [t.name, t.value]));
  ...
  const decryptedShards: Array<{ version: number; shardJson: string }> = [];
